...
uv run streamlit run run_src.statflow.py
```

Or run directly with:

```bash
uv run python run_src.statflow.py
```
...
User preferences (colors, symbols, graph settings) are automatically saved to `~/.src.statflow/config.yaml`.
...
The original monolithic `src.statflow.py` remains unchanged and can still be used with:

```bash
uv run streamlit run src.statflow.py
```
