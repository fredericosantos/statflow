...
from statflow.pages_modules.module_plot_macros.plot_data_fetcher import fetch_plot_data
...
st.set_page_config(
    page_title=f"Plot Macros - {st.session_state['app_name']}",
...
def sanitize_dataset_name(name: str) -> str:
    ...
    renames = st.session_state['dataset_renames']
...
def generate_csv_data_for_plot_type(all_runs_df, plot_type, selected_datasets, selected_configs, config_mappings):
    ...
    for dataset_name in selected_datasets:
        ...
        if plot_type == "Boxplot":
            ...
            # For convergence plots, fetch metric history for selected runs
            if selected_metric:
                # Get run IDs for selected configs
                run_ids = []
                for config in selected_configs:
                    config_data = dataset_df[dataset_df["config_group"] == config]
                    if not config_data.empty:
                        run_ids.extend(config_data["run_id"].tolist())
                
                if run_ids:
                    # Fetch convergence data
                    convergence_df = get_multiple_metric_histories(run_ids, selected_metric)
                    
                    if not convergence_df.is_empty():
                        # Aggregate by step and config
                        # This is simplified - in practice, need to map run_id back to config
                        # For now, create a basic structure
                        csv_df = convergence_df.group_by("step").agg([
                            pl.col("value").mean().alias("mean_fitness"),
                            pl.col("value").std().alias("std_fitness")
                        ]).sort("step").to_pandas()
                        ...
                        csv_df = pd.DataFrame({
                            ...
                            "mean_fitness": [float('nan')] * 10,
                            "std_fitness": [float('nan')] * 10
                        })
                        csv_files[f"{dataset_name}.csv"] = csv_df.to_csv(index=False, sep=' ')
                else:
                    csv_df = pd.DataFrame({
...
def main():
    ...
    if st.button("← Back to Home", key="back_home"):
        st.switch_page("subpages/get_started.py")
    ...
    with st.sidebar:
        ...
        # Metric selection for line plots
        selected_metric = None
        if plot_type == "Line (with Error Bands)":
            st.markdown("### Convergence Metric")
            # Get available metrics from selected datasets
            if selected_datasets:
                from statflow.functional.mlflow.mlflow_client import get_metrics_from_experiments
                # Get experiment names - this needs to be implemented properly
                # For now, placeholder
                experiment_names = []  # TODO: Fetch experiment names for selected datasets
                if experiment_names:
                    available_metrics = get_metrics_from_experiments(tuple(experiment_names))
                    if available_metrics:
                        selected_metric = st.selectbox(
                            "Select Metric for Convergence Plot:",
                            options=available_metrics,
                            help="Choose the metric to plot over training steps"
                        )
                    else:
                        st.warning("No metrics found in selected experiments.")
                else:
                    st.info("Select datasets to see available metrics.")
            else:
                st.info("Select datasets to see available metrics.")
    ...
    from statflow.functional.dataframes.data_processing import create_group_label
    ...
    with col_config:
        ...
        for idx, config in enumerate(available_configs):
            ...
            with col_picker:
                ...
                selected_color = st.session_state[f"color_{config}"]
        ...
        params = {
            ...
            "ylabel": ylabel,
            "linewidth": linewidth,
            "gap_after": st.session_state["boxplot_gap_after"],
            "gap_size": st.session_state["boxplot_gap_size"],
        }
