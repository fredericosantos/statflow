...
from statflow.functional.table_builders.statistical_table_builder import build_statistical_table
from statflow.functional.table_builders.nodes_table_builder import build_nodes_table
from statflow.functional.table_builders import (
...
from statflow.pages_modules.module_multiple_datasets.data_aggregator import fetch_and_process_multiple_datasets
from statflow.pages_modules.module_multiple_datasets.comparison_processor import prepare_comparison_tables


st.set_page_config(
    page_title=f"Multiple Datasets Comparison - {st.session_state['app_name']}",
...
def main():
    ...
    with tab_significance:
        with st.spinner("Building statistical significance table..."):
            datasets_path = st.session_state['datasets_path']
                        ...
                        if not detailed_df.empty:
                            render_table_with_downloads(
                                ...
                                **Legend:** — = Best performing configuration (no self-comparison)
                                """,
                                latex_rotation_angle=st.session_state['latex_rotation_angle'],
                                latex_font_size=st.session_state['latex_font_size']
                            )
                        else:
    ...
    with tab_names:
        ...
        current_renames = st.session_state['dataset_renames']
