...
Usage:
    from statflow.components.visualization import (
...
def get_pareto_color(group_label: str, custom_colors: dict[str, str] | None = None) -> str:
    ...
    if custom_colors:
        if "GSGP-OMS" == group_label:
            return custom_colors["GSGP-OMS"] if "GSGP-OMS" in custom_colors else "#5A6C7D"
        elif "GSGP-std" == group_label:
            return custom_colors["GSGP-std"] if "GSGP-std" in custom_colors else "#B8C5D0"
        elif "SLIM" == group_label:
            return custom_colors["SLIM"] if "SLIM" in custom_colors else "#D4A574"
        # ARC colors by beta value
        arc_beta_colors = custom_colors["arc_beta"] if "arc_beta" in custom_colors else {}
...
def get_marker_symbol(group_label: str, custom_symbols: dict[str, str] | None = None) -> str:
    ...
    if custom_symbols:
        if "GSGP-OMS" == group_label:
            return custom_symbols["gsgp_oms"] if "gsgp_oms" in custom_symbols else "circle"
        elif "GSGP-std" == group_label:
            return custom_symbols["gsgp_std"] if "gsgp_std" in custom_symbols else "square"
        elif "SLIM" == group_label:
            return custom_symbols["slim"] if "slim" in custom_symbols else "diamond"
        arc_beta_symbols = custom_symbols["arc_beta"] if "arc_beta" in custom_symbols else {}
...
def get_color_for_config(row: pd.Series, custom_colors: dict[str, str] | None = None) -> str:
    ...
    """
    variant = row["params.variant"] if "params.variant" in row else "N/A"

    # Use custom colors if provided
    if custom_colors:
        if variant == "gsgp":
            use_oms = row["params.use_oms"] if "params.use_oms" in row else "False"
            if use_oms == "True":
                return custom_colors["GSGP-OMS"] if "GSGP-OMS" in custom_colors else "#5A6C7D"
            else:
                return custom_colors["GSGP-std"] if "GSGP-std" in custom_colors else "#B8C5D0"
            ...
            beta_str = row["params.arc_beta"] if "params.arc_beta" in row else "0.0"
            arc_beta_colors = custom_colors["arc_beta"] if "arc_beta" in custom_colors else {}
            return arc_beta_colors[beta_str] if beta_str in arc_beta_colors else "#9B8590"

    # Fallback to original logic with MPF-based colors
    if variant == "arc":
        # Get MPF and beta values
        mpf_str = row["params.mutation_pool_factor"] if "params.mutation_pool_factor" in row else "10"
        beta_str = row["params.arc_beta"] if "params.arc_beta" in row else "0.0"
        ...
        p_inflate_str = row["params.arc_beta"] if "params.arc_beta" in row else "0.5"
