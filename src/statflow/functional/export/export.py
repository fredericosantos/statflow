...
from statflow.components.data_processing import create_group_label, get_sort_key
...
def create_fitness_zip(
    ...
    unique_datasets = all_runs_df['dataset_name'].unique()
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        for i, dataset in enumerate(unique_datasets):
            ...
            if progress_bar:
                progress = 0.1 + (0.9 * (i + 1) / len(unique_datasets))
...
def create_dataset_csv(dataset_runs: pl.DataFrame, config_order: list[str]) -> str:
    ...
    for config in config_order:
        config_runs = dataset_runs.filter(pl.col("config_group") == config)

        if not config_runs.is_empty():
            for run in config_runs.iter_rows(named=True):
                row = {
                    "config": config,
                    "run_id": run["run_id"] if "run_id" in run else "",
                    "rmse": run["metrics.best_test_fitness"] if "metrics.best_test_fitness" in run else "",
                    "tree_size": run["metrics.best_n_nodes"] if "metrics.best_n_nodes" in run else "",
                    "variant": run["params.variant"] if "params.variant" in run else "",
                    "arc_beta": run["params.arc_beta"] if "params.arc_beta" in run else "",
                    "mutation_pool_factor": run["params.mutation_pool_factor"] if "params.mutation_pool_factor" in run else "",
                    "use_oms": run["params.use_oms"] if "params.use_oms" in run else "",
                    "new_oms": run["params.new_oms"] if "params.new_oms" in run else "",
                    "pool_refresh_interval": run["params.pool_refresh_interval"] if "params.pool_refresh_interval" in run else "",
                }
                csv_data.append(row)
    ...
    return csv_df.write_csv()


def export_table_to_csv(df: pl.DataFrame, filename: str = "table.csv") -> str:
...
def export_table_to_latex(
    ...
    dataset_renames: dict[str, str | dict[str, str]] | None = None
) -> str:
...
def _get_display_name(dataset: str, renames: dict[str, str | dict[str, str]]) -> str:
    ...
    if rename_entry is None:
        ...
        display_name = rename_entry.get("display_name", dataset)
    else:
        display_name = rename_entry
...
def _export_rmse_table_latex(
    ...
    rotation_angle: int,
    font_size: str,
    dataset_renames: dict[str, str | dict[str, str]]
) -> str:
