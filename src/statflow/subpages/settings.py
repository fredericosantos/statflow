...
from statflow.config import (
    State, DEFAULT_PARETO_COLORS, DEFAULT_PARETO_SYMBOLS, DEFAULT_GRAPH_CONFIG, DEFAULT_APP_NAME, DEFAULT_DATASETS_PATH
)
from statflow.pages_modules.shared.server_status import ServerStatusManager


st.set_page_config(
    page_title=f"Settings - {st.session_state['app_name'] if 'app_name' in st.session_state else DEFAULT_APP_NAME}",
...
def main():
    ...
    app_name = st.text_input(
        "🏷️ Application Name",
        value=st.session_state['app_name'] if 'app_name' in st.session_state else DEFAULT_APP_NAME,
        max_chars=50,
        help="This name will be displayed in page titles and headers throughout the application."
    )

    if app_name != (st.session_state['app_name'] if 'app_name' in st.session_state else DEFAULT_APP_NAME):
    ...
    datasets_path = st.text_input(
        "📁 Datasets Path",
        value=st.session_state['datasets_path'] if 'datasets_path' in st.session_state else DEFAULT_DATASETS_PATH,
        help="Path to the directory containing dataset CSV files."
    )

    if datasets_path != (st.session_state['datasets_path'] if 'datasets_path' in st.session_state else DEFAULT_DATASETS_PATH):
    ...
    with tab_colors:
        ...
        with variant_cols[0]:
            ...
            with cols[0]:
                gsgp_std_color = st.color_picker(
                    "GSGP Standard",
                    value=st.session_state['custom_colors']['GSGP-std'],
                    key="gsgp_std_color"
                )
            ...
            with cols[1]:
                gsgp_oms_color = st.color_picker(
                    "GSGP-OMS",
                    value=st.session_state['custom_colors']['GSGP-OMS'],
                    key="gsgp_oms_color"
                )
        ...
        with variant_cols[1]:
            ...
            slim_color = st.color_picker(
                "SLIM",
                value=st.session_state['custom_colors']['SLIM'],
                key="slim_color"
            )
        ...
        for i, beta in enumerate(beta_values):
            with cols[i % 8]:
                ...
                current_color = st.session_state['custom_colors']['arc_beta'][beta]
    ...
    with tab_symbols:
        ...
        with col1:
            gsgp_std_symbol = st.selectbox(
                ...
                index=symbol_options.index(st.session_state['custom_symbols']['gsgp_std']),
                key="gsgp_std_symbol"
            )
        ...
        with col2:
            gsgp_oms_symbol = st.selectbox(
                "GSGP-OMS Symbol",
                options=symbol_options,
                index=symbol_options.index(st.session_state['custom_symbols']['gsgp_oms']),
                key="gsgp_oms_symbol"
            )
        ...
        slim_symbol = st.selectbox(
            "SLIM Symbol",
            options=symbol_options,
            index=symbol_options.index(st.session_state['custom_symbols']['slim']),
            key="slim_symbol"
        )
        ...
        for i, beta in enumerate(beta_values):
            with cols[i % 4]:
                ...
                current_symbol = st.session_state['custom_symbols']['arc_beta'][beta]
    ...
    with st.expander("🔍 Current Configuration Preview", expanded=False):
        st.json({
            "use_custom_colors": st.session_state['use_custom_colors'],
            "show_mean": st.session_state['show_mean'],
            "show_error_bars": st.session_state['show_error_bars'],
            "graph_width": st.session_state['graph_width'],
            "graph_height": st.session_state['graph_height'],
